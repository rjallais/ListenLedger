...
func (h *Handler) RegisterRoutes(r *router.Router[*core.RequestEvent]) {
	...
	r.POST("/api/albums/{albumId}/collection/{action}", h.handleUpdateAlbumSongField(clampAlbumCollectionSongs))
	r.POST("/api/albums/{albumId}/total/{action}", h.handleUpdateAlbumSongField(clampAlbumTotalSongs))
